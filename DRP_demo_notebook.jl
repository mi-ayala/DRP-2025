...
# ╔═╡ df10d727-e187-4aea-ab79-ae632f6c938b
begin
	using PlutoUI
	using LaTeXStrings
	using Plots
...
# ╔═╡ b694fbdc-0cf6-4c71-96b5-5069f10fa336

md" N : $(@bind N Select([3, 25, 50, 75, 100]))"

# ╔═╡ abcd7b79-21d3-4f9b-8f2f-e045b4135e53

md" λ : $(@bind λ Select([0.01, 0.03, 0.05]))"

# ╔═╡ d02e640a-156d-11f0-2c72-4d6fd300bab9

# begin
   
#     # λ = 0.05

#     # Define the non-normalized potential
#     potential(r, α, λ) = (1/α) * r.^α .+ (1/λ) * r.^(-λ)

#     # Range for r (avoid 0 to prevent singularity in r^(-λ))
#     r_vals = range(1e-6, stop=2, length=1000)

#     ys = potential(r_vals, α, λ)

#     # fig = Figure()
#     # ax = Axis(fig[1, 1];
#     #     xlabel = L"r",
#     #     ylabel = L"K(r)",
#     #     title = L"K(r)\ \text{for}\ \alpha = %$α",
#     #     xticklabelsize = 12,
#     #     yticklabelsize = 12,
#     #      # limits = ((0,1.5), (0, 40))
#     # )

#     plot( r_vals, ys, color = :blue, linewidth = 2, label = L"K(r)")
...
# ╔═╡ 327f964d-953e-4551-97ca-1971005c579b
md"Integration"

# ╔═╡ cffc0a09-abf1-454a-a558-db28e945dd39
md"Continuation"

# ╔═╡ c9da76a9-4c36-4edf-8a61-8e81618e9195
md" α : $(@bind α PlutoUI.Slider(20:1:100, default=20, show_value=true))"

# ╔═╡ 81a8c444-dfa3-482b-bab7-0bff926dba8d
begin
	function plot_particles(
	    ...
	    xs, ys, zs = x[:, 1], x[:, 2], x[:, 3]
	    xmin, xmax = minimum(xs), maximum(xs)
	    ymin, ymax = minimum(ys), maximum(ys)
	    zmin, zmax = minimum(zs), maximum(zs)
	    radii = sqrt.(xs .^ 2 .+ ys .^ 2 .+ zs .^ 2)  # Distance to origin
	    radii_norm = (radii .- minimum(radii)) ./ (maximum(radii) - minimum(radii) + eps())
	    ...
	    # Colored scatter
	    Plots.scatter3d!(xs, ys, zs;
	       	#zcolor=radii_norm,
	        markersize=marker_size,
	        markerstrokewidth=0.0,
			c = :blue,
			opacity=0.6# Change this if you want to change colors
	    )
	
		# Title
	    Plots.title!( "α = $alpha,\nγ = $gamma, N = $n_particles")
	
	    return p
	end
	
	function get_state_for_continuation(u, N, ff, α, λ)
	  ...
	  ### Newton Refinement
	  u = reshape(u, N, 3)
	  u .-= mean(reshape(u, N, 3), dims=1)
	
	  return u
	end
end

# ╔═╡ 44d58352-1911-40c4-b429-01a5311b90bd
begin
	function g!(du, u, p, t)
...
# ╔═╡ 9fb5165f-a3df-4292-b8e0-cc91406f39e8
begin



	ff = ODEFunction(g!; jac=h!)
	u = vec(2 * rand(3, N) .- 1)
	u = reshape(u, N,3)

	u_ic = get_state_for_continuation(u, N, ff, 20, λ) ### initial condition with alpha =20
	
    p1 = plot_particles(u; alpha=20, gamma=λ, n_particles=N)
    p2 = plot_particles(u_ic; alpha=20, gamma=λ, n_particles=N)

    Plots.plot(p1, p2, layout = (1, 2), size = (800, 400))
end


# ╔═╡ 223545c5-1dba-4257-9127-fd6386e660c1
function solver(α,λ,u)
	...
	Ya = zeros(type, N, N)
    p = (α, λ, N, r, f, a, XX, YY, ZZ, YX, ZX, ZY, Xa, Ya)
    u = solve(SteadyStateProblem(ff, u[:], p), DynamicSS(TRBDF2()))
    u = reshape(u,N,3)
    #println("norm when α = $α: $(norm(g(u,p), Inf))")
    return u
end

# ╔═╡ 60d8ae52-d8c2-41bd-aeb4-e4c9c3156842
begin
	u0 = vec(2 * rand(3, N) .- 1)
	...
	u_continuation = u ### This should be the output of your continuation function. The input of the continuation function should be u_ic, with parameters alpha=20 and λ(this is the variable in the notebook). The final alpha should be α(variable of the notebook) 
	plot_particles(u; alpha=α, gamma=λ, n_particles=N)
end
...
manifest_format = "2.0"
project_hash = "9bd39c0b468fc8d97f0b2bbd5a1d641beea22394"
